...
<script type="text/javascript">
	function id_check(){
		...
		window.open("./controller?cmd=viewpoint"+"&id="+checkid,"","width=300px height=200px");
		return false;
	}
	
	function zip_check(){
		var checkid1 = document.getElementById("userzip1").value;
		var checkid2 = document.getElementById("userzip2").value;
		var checkid3 = document.getElementById("useraddr1").value;
		var checkid4 = document.getElementById("useraddr2").value;
		window.open("./make_zipcode.jsp?zip1="+checkid1+"&zip2"+checkid2+"&addr1"+checkid3+"&addr2"+checkid4,"","width=300px height=200px");
		return false;
	}
	function id_check_with_ajax(val){
		...
		xhr.onreadystatechange = function(){
			if(this.readyState==4&&this.status==200){
				...
				///ajax를 통해 여러가지 데이터를 받는 방식 데시로 해도되지만 이런방식이 있다고 보여준다.
				if(val=="0"){
...
<table>
	...
	<tr><td>우편번호</td>
		<td>
			<input type="text" name="zip1" size="3" value="<%=vo.getZipcode().split("-")[0]%>" id="userzip1"> -
			<input type="text" name="zip2" size="3" value="<%=vo.getZipcode().split("-")[1]%>" id="userzip2">
			<span id="zipmsg"></span>
		</td>
		<td><button onclick=" return zip_check()">우편번호검사</button></td>
		<td><input type="hidden" name="isZipCheck" value="false" id="isZipCheck"></td></tr>
	<tr><td>주소1</td><td><input type="text" name="addr1" size="30" value="<%=vo.getAddr1()%>" id="useraddr1"></td><td></td><td></td></tr>
	<tr><td>주소2</td><td><input type="text" name="addr2" size="30" value="<%=vo.getAddr2()%>" id="useraddr2"></td><td></td><td></td></tr>
